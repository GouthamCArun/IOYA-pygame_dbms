...
style=ttk.Style()
root.title("Let the Struggle Begin")


open_eye=ImageTk.PhotoImage(Image.open(r"show_password.png").resize((30,30*(643//388)),resample=0))
close_eye=ImageTk.PhotoImage(Image.open(r"hide_password.png").resize((30,30*(643//388)),resample=0))
#Setting window icon
...
def login():
    ...
    if name in row[0]:
        user_flag=True
        cursor.execute("SELECT PasswordHash FROM Users where Username=?", (name,))
        row = cursor.fetchone()
        fetch_passwd = row[0]
        if fetch_passwd==pwd:
            pass_flag=True
...
login_Label=ttk.Label(login_page,text="\tIOYA: Login\t",anchor=CENTER,font=("Times New Roman",20),width=30)
